...
#pragma message "SSE4.2 Enabled"
	...
	for( ; i < Count / 4; ++i )
	{
		...
		__m128i CurDistances = _mm_loadu_si128(
			reinterpret_cast<const __m128i*>(&Distances[i])
		);
...
#pragma message "NEON Enabled"
	...
	for( ; i < Count / 4; ++i )
	{
		...
		uint32x4_t CurDistances = vld1q_u32(
			reinterpret_cast<const std::uint32_t*>(&Distances[i])
		);
		...
		vst1q_u32(
			reinterpret_cast<uint32_t*>(&Positions[i]),
			Interleaved.val[0]
		);
		vst1q_u32(
			reinterpret_cast<uint32_t*>(&Positions[i + 2]),
			Interleaved.val[1]
		);
	}
#endif
...
#else
	const std::uint32_t Depth = __builtin_clz(Size) - 1;
#endif
	/// 8 at a time ( AVX 2 )
	for( ; i < Count / 8; ++i )
	{
		...
		__m256i CurDistances = _mm256_loadu_si256(
			reinterpret_cast<const __m256i*>(&Distances[i])
		);
		...
		_mm256_storeu_si256(
			reinterpret_cast<__m256i*>(&Positions[i]),
			PermuteFirst
		);
		// store second four ((x,y),(x,y),(x,y),(x,y))
		_mm256_storeu_si256(
			reinterpret_cast<__m256i*>(&Positions[i + 4]),
			PermuteSecond
		);
	}
#endif
...
)
{
	...
	for( ; i < (Count-i) / 16; ++i )
	{
		...
		__m512i CurDistances = _mm512_loadu_si512(
			reinterpret_cast<const __m512i*>(&Distances[i])
		);
		...
		_mm512_storeu_si512(
			reinterpret_cast<__m512i*>(&Positions[i]),
			PermuteFirst
		);
		// store second eight ((x,y),(x,y),(x,y),(x,y),(x,y),(x,y),(x,y),(x,y)
		_mm512_storeu_si512(
			reinterpret_cast<__m512i*>(&Positions[i + 8]),
			PermuteSecond
		);
	}
